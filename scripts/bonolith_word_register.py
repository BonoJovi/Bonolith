...
"""Bonolith word register / edit dialog (GTK3).
...
class _IBusActivator:
    """Turn Bonolith 日本語ON via its session-bus control surface.
    ...
    Instead Bonolith exposes `org.bonolith.Control` on the session bus. `ForceEnable`
    ...
    _NAME = "org.bonolith.Control"
    _PATH = "/org/bonolith/Control"
...
class WordDialog(Gtk.Dialog):
    def __init__(self, activator, mode, init_reading, init_surface):
        title = "Bonolith: 単語登録" if mode == "register" else "Bonolith: 単語編集"
...
def main(argv):
    parser = argparse.ArgumentParser(prog="bonolith_word_register.py")
