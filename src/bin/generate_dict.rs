...
fn extra_entries() -> Vec<Entry> {
    let extras: &[(&str, &str, &str, u32)] = &[
        ...
        ("ご", "誤", "Noun", 5000),
        ("おう", "応", "Noun", 5500),  // 応用・応答・対応 etc.; IPADIC漏れ
        ("のう", "応", "Noun", 5000),  // 反応 etc.; IPADIC漏れ
        // サ変接続名詞+する の分節誤りを防ぐ複合エントリ
        // 問題: "きょうゆうする" → 今日(6310)+有する(3281) が 共有(4435)+する(1306) に勝つ
        ("きょうゆう", "共有", "Noun", 7500),  // freq≥7212 ないと今日+有するに負ける
        ("きょうゆうする", "共有する", "Verb", 7500),
        ("きょうゆうした", "共有した", "Verb", 7000),
        ("きょうゆうして", "共有して", "Verb", 7000),
        ("きょうゆうしない", "共有しない", "Verb", 6500),
        ("きょうゆうします", "共有します", "Verb", 7000),
        // IT用語+する 複合エントリ: user_scorer の "こみ" ブーストで分節が壊れるのを防ぐ
        // "こみっとしましょう" → 込(こみ learned)+っと+しましょう になるバグへの対処
        ("こみっとする", "コミットする", "Verb", 7000),
        ("こみっとします", "コミットします", "Verb", 7000),
        ("こみっとしましょう", "コミットしましょう", "Verb", 7000),
        ("こみっとした", "コミットした", "Verb", 6500),
        ("こみっとして", "コミットして", "Verb", 6500),
        ("こみっとしない", "コミットしない", "Verb", 6500),
        // 変換: 4429(IPADIC) は低すぎる。"変(へん learned)+冠します" に負けるため大幅ブースト
        ("へんかん", "変換", "Noun", 8000),
        ("へんかんする", "変換する", "Verb", 7500),
        ("へんかんします", "変換します", "Verb", 7500),
        ("へんかんした", "変換した", "Verb", 7000),
        ("へんかんして", "変換して", "Verb", 7000),
        ("へんかんしない", "変換しない", "Verb", 6500),
